...
import { Component, Braces, Zap, Code2, Layers } from 'lucide-react';

interface PluginProps {
  onInsert: (text: string) => void;
}

export const JsxPlugin: React.FC<PluginProps> = ({ onInsert }) => {
  const snippets = [
    { label: 'Component', icon: Component, code: 'export function Component(props) {\n  return <div></div>;\n}' },
    ...
    { label: 'Context', icon: Layers, code: 'const value = useContext(MyContext);' },
  ];

  return (
    <div className="flex items-center gap-1 overflow-x-auto p-1 custom-scrollbar">
      <span className="text-[10px] uppercase font-bold text-sky-500 mr-2 flex-shrink-0">JSX Tools</span>
