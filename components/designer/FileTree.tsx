    ...
    >
      ...
      {rootType === 'models' && !targetItem && (
         <>
           ...
           >
              <Database size={14} className="text-amber-500" /> 连接数据库
           </button>
           <div className="h-px bg-gray-100 dark:bg-gray-700 my-1"></div>
         </>
      )}

      {/* Root / Folder Actions (Excluding dbConnection handled above) */}
      {isFolder && targetItem?.type !== 'dbGroup' && rootType !== 'models' && ( 
        <>
          <button 
            className="w-full text-left px-4 py-2 hover:bg-nebula-50 dark:hover:bg-gray-700 flex items-center gap-2"
